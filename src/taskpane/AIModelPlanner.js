...
async function saveModelCodesToLastModelFile(modelCodesString) {
    try {
        ...
        if (modelPlannerConversationHistory.length > 0) {
            modelPlannerConversationHistory.forEach(([role, message], index) => {
                conversationSection += `${role.toUpperCase()}:\n${message}\n\n`;
            });
        } else {
            conversationSection += "No conversation history available.\n\n";
        }
        
        // 2. Get training data from localStorage
        let trainingDataSection = "=== TRAINING DATA ===\n\n";
        try {
            const trainingQueue = localStorage.getItem('trainingDataQueue');
            if (trainingQueue) {
                const trainingData = JSON.parse(trainingQueue);
                if (trainingData && trainingData.length > 0) {
                    trainingData.forEach((entry, index) => {
                        trainingDataSection += `Entry ${index + 1}:\n`;
                        if (entry.prompt) {
                            trainingDataSection += `User Input: ${entry.prompt}\n`;
                        }
                        if (entry.selectedCode) {
                            trainingDataSection += `AI Response: ${entry.selectedCode}\n`;
                        }
                        trainingDataSection += "\n";
                    });
                } else {
                    trainingDataSection += "No training data found.\n\n";
                }
            } else {
                trainingDataSection += "No training data found.\n\n";
            }
        } catch (e) {
            console.warn("[saveModelCodesToLastModelFile] Error reading training data:", e);
            trainingDataSection += "Error reading training data.\n\n";
        }
        
        // 3. Get custom prompts and attached files
        let customPromptsSection = "=== CUSTOM PROMPTS AND ATTACHMENTS ===\n\n";
        
        // Get the system prompt that was used
        const systemPromptMode = getSelectedSystemPromptMode();
        customPromptsSection += `System Prompt Mode: ${systemPromptMode}\n\n`;
        
        // Add attached files information
        if (currentAttachedFiles.length > 0) {
            customPromptsSection += "Attached Files:\n";
            currentAttachedFiles.forEach((file, index) => {
                customPromptsSection += `\nFile ${index + 1}: ${file.fileName}\n`;
                customPromptsSection += `Type: ${file.fileType}\n`;
                customPromptsSection += `Size: ${file.fileSize}\n`;
                
                // Include file content if it's text-based
                if (file.fileType !== 'Image' && file.content) {
                    customPromptsSection += `Content:\n${file.content}\n`;
                } else if (file.fileType === 'Image') {
                    customPromptsSection += `Image Data: [Base64 image data excluded for brevity]\n`;
                }
            });
        } else {
            customPromptsSection += "No files were attached.\n";
        }
        customPromptsSection += "\n";
        
        // 4. Get the JSON response that was generated
        let jsonResponseSection = "=== JSON RESPONSE ===\n\n";
        
        // Find the last assistant response that contains JSON
        let jsonResponse = null;
        for (let i = modelPlannerConversationHistory.length - 1; i >= 0; i--) {
            const [role, message] = modelPlannerConversationHistory[i];
            if (role === 'assistant') {
                try {
                    // Try to parse as JSON
                    const cleanedMessage = stripCodeBlockMarkers(message);
                    const parsed = JSON.parse(cleanedMessage);
                    if (parsed && parsed.ModelCodes) {
                        jsonResponse = parsed;
                        break;
                    }
                } catch (e) {
        ...
        if (jsonResponse) {
            jsonResponseSection += JSON.stringify(jsonResponse, null, 2) + "\n\n";
        } else {
            jsonResponseSection += "No JSON response found in conversation history.\n\n";
        }
        
        // 5. Add the ModelCodes section
        let modelCodesSection = "=== MODEL CODES ===\n\n";
        modelCodesSection += modelCodesString + "\n\n";
        
        // Combine all sections
        const contentToSave = `// Model generation session - ${timestamp}
...
${modelCodesSection}`;
        ...
        try {
            ...
            localStorage.setItem('lastModelSession', JSON.stringify({
                timestamp: timestamp,
                conversation: modelPlannerConversationHistory,
                trainingData: localStorage.getItem('trainingDataQueue'),
                attachedFiles: currentAttachedFiles,
                jsonResponse: jsonResponse,
                modelCodes: modelCodesString,
                systemPromptMode: systemPromptMode
            }));
            console.log("[saveModelCodesToLastModelFile] Full session data stored in localStorage");
        } catch (storageError) {
        ...
        console.log("[saveModelCodesToLastModelFile] Full session content for Last Model.txt:\n", contentToSave);
        ...
        try {
            ...
            a.href = url;
            a.download = 'Last Model - Full Session.txt';
            document.body.appendChild(a);
            a.click();
            document.body.removeChild(a);
            URL.revokeObjectURL(url);
            
            console.log("[saveModelCodesToLastModelFile] Full session file download initiated");
            displayInClientChatLogPlanner("Full session data saved! A download should have started for 'Last Model - Full Session.txt'. This includes conversation history, training data, custom prompts, JSON response, and model codes.", false);
        } catch (downloadError) {
            console.warn("[saveModelCodesToLastModelFile] Download creation failed:", downloadError);
            displayInClientChatLogPlanner("Full session data generated. Please check the console and copy the content to manually save it.", false);
        }
        
    } catch (error) {
        console.error("[saveModelCodesToLastModelFile] Error saving session data:", error);
        displayInClientChatLogPlanner(`Error saving session data: ${error.message}`, false);
    }
}
